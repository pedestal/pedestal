...
(defmacro ^{:private true} defsimpleinterceptordef
  ...
  [n docstring]
  (let [sym       (symbol (str "def" (name n)))
        sym-in-ns (str *ns* "/" sym)]
    `(defmacro ~sym
       ~docstring
       [macro-name# & args#]
       (deprecated ~sym-in-ns
...
(defn on-request
  ...
  ([f] (deprecated `on-request
         (before (fn [context]
                   (assoc context :request (f (:request context)))))))
  ([f & args]
   (deprecated `on-request
     (let [[n f args] (if (fn? f)
                        [nil f args]
                        [f (first args) (rest args)])]
       (interceptor {:name  (interceptor-name n)
...
(defn on-response
  ...
  ([f] (deprecated `on-response
         (after (fn [context]
                  (assoc context :response (f (:response context)))))))
  ([f & args]
   (deprecated `on-response
     (let [[n f args] (if (fn? f)
                        [nil f args]
                        [f (first args) (rest args)])]
       (interceptor {:name  (interceptor-name n)
