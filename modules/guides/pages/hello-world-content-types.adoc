...
link:https://github.com/pedestal/pedestal/tree/master/modules/guides/examples/hello-content[the
...
The slightly tricky `cond->`{empty}foonote:[That's read out loud as "cond arrow". Once you master it,
you'll really level up in Clojure skills.] macro is what we need here.
`cond->` is similar to the usual
threading macro `->`, but it stead of a series of expressions, it expectes  pairs of conditions and expressions; the condition is evaluated, and only if true, is the value threaded through the expression.
