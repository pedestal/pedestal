...
traces more helpful.)
2. The table does not have hierarchical nesting, so the rows are
independent.
...
See the xref:reference:routing-quick-reference.adoc[] for full details.
...
An an alternative to using path contrains, you could define a validating interceptor:
...
When you reuse interceptors, or when you supply a handler _function_ as the
final interceptor, you'll need to explicitly specify the route name.
