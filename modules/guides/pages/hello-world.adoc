...
of the link:hello-world-content-types[whole response]. For now we're
...
This trivial `response-hello` handler function is functional. It can be tested entirely
by invoking it, passing in request map, and looking at the response map it returns.
There's no need to start up a server and send a request to it via HTTP, you can just
have testing code call this code.
...
NOTE:: The downloads only occur the _first_ time you run the service; you can also see
that pedestal.jetty bring in further dependencies on the other parts of pedestal (pedestal.log,
pedestal.service, etc.)

The `clj` tool will download dependencies, add them to the classpath
and start a repl in the `user` namespace. Now we're able to 
input Clojure code to evaluate it (turn it into a value). The first
...
This is perfectly acceptable for testing; in fact we could have passed a `nil` as the
`respond-hello` function doesn't actually use the request map.
...
link:your-first-api#_interactive_development[run on its own thread] so
...
link:https://github.com/pedestal/pedestal/tree/master/modules/guides/examples/hello[this guide].
