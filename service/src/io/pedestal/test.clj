  ...
  (let [{:keys [scheme host path query-string]} (parse-url url)
    ...
    (with-meta
      (reify HttpServletRequest
        ...
        (getContentLength [this]  (int (read-string (get-in options [:headers "Content-Length"] "0"))))
        (getContentLengthLong [this] (long (read-string (get-in options [:headers "Content-Length"] "0"))))
