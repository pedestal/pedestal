...
(defn generic-modal-collect-input [parent-id id dispatcher transform-name messages]
...
(defn modal-collect-input [r dispatcher path transform-name messages]
      ...
      (if (seq syms)
        ...
        (event/listen! (d/by-id button-id)
                       ...
                       (fn [e]
                         ...
                         (modal-collect-input r dispatcher path transform-name messages)))
...
(defn attach-click-event [id transform-name messages dispatcher]
  (let [messages (map (partial msg/add-message-type transform-name) messages)]
           ...
           (if (seq syms)
             (event/listen! (d/by-id item-id)
                            ...
                            (fn [e]
                              ...
                              ;; deleted.
                              (modal-collect-input r dispatcher modal-path transform-name messages)))
