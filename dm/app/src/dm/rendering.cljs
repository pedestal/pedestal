(ns dm.rendering
  ...
  (:require-macros [dm.html-templates :as html-templates]))

;; Load templates.

(def templates (html-templates/dm-templates))
...
(defn render-page [renderer [_ path] transmitter]
  (let [;; The renderer that we are using here helps us map changes to
        ...
        ;; Get the dynamic template named :dm-page
        ...
        html (templates/add-template renderer path (:dm-page templates))]
