...
You can type the full name if you like, but those long namespace names add up quickly, especially
if you are calling multiple functions from the namespace. And alias makes the code more concise and
easier to read.
...
in the `user` namespace. Now we're able to
...
There's an informal contract here:  Pedestal expects that `hello-handler` is a function that can accept a single parameter, a request map.  Further, it expects that, as a handler function, it will return a response map.  Violating
