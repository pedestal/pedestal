...
namespace; a `/` separates the namespace alias from the symbol. `conn/default-connector-map` is the same as
`io.pedestal.connector/default-connector-map`, just with less for you to type.

You may type the fully qualified name if you like, but those long namespace names add up quickly, especially
if you are calling multiple functions from the namespace. And alias makes your code more concise and
easier to read.
...
pairs of keys and values.  Likewise, a _vector_ is marked by square brackets (`[` and `]`) and a general _list_ by `(` and `)` and a set by `#{` and `}`.
====
