...
- Setup and run a HTTP server around the route
...
xref:embedded-template.adoc[template]. But for now, it's more important to see what the pieces are
...
you can think of a namespace as similar to a package name.
...
Speaking of the stuff we require, what is `io.pedestal.connector` and
`io.pedestal.http.http-kit`? Those are each namespaces from one of
the Pedestal libraries.

The
api:*[ns=io.pedestal.connector]
namespace has functions that used to describe a Pedestal connector.
The description starts with the port number to listen to, and continues
from there to describe the behavior of the connector, in Pedestal terms,
by describing the routes and interceptors that handle incoming requests.
...
alias inside our namespace.

An alias is a prefix that can be used to name symbols defined inside a
namespace; a `/` separates the namespace alias from the symbol inside the namespace. `conn/default-connector-map` is the same as
`io.pedestal.connector/default-connector-map`, just with less to type,
especially when your namespace calls many different functions
in the aliased namespace.

Also, don't worry about the indentation ... Clojure
ignores whitespace outside of quoted strings entirely. Many common
...
- Next, the routing information that Pedestal uses to connect requests to handler functions
...
----
<1> Define  a function, using `defn`,  called `hello-handler` that takes a single argument, which we will call `_request`
...
pairs of keys and values.  Likewise, a _vector_ is marked by square brackets (`[` and `]`) and a general _list_ by `(` and `)`.
...
A handler function, such as `hello-handler`, returns a xref:reference:response-map.adoc[].
...
but `hello-handler` is so trivial that it ignores the request;
the `_` prefix on the parameter name, `_request`,
is a Clojure convention for parameters that are present but purposely ignored.

This trivial `hello-handler` function is functional -- it can be tested entirely
...
stable, and minimal HTTP server. Best of all, it doesn't require any
...
The exact libraries and versions downloaded will vary with the exact version of Pedestal.
...
as the `hello-handler` function doesn't actually use the `_request` parameter.
...
When routed in a live service, the `hello-handler` function will be invoked.
We've named this route :greet; every route must have a unique
name, though in many cases, Pedestal can automatically provide a reasonable name from other information you provide in the route.

There's a kind of informal contract here:  Pedestal expects that `hello-handler` is a function that can accept a single parameter, a request map.  Further, it expects that, as a handler function, it will return a response map.  Violating
...
----
<1> A connector is built from a connector map; this builds a map with the specified port number.
...
`with-routes`, and so forth. This is _very_ useful, concise, and common in Clojure code.
...
The `start!` function returns the connector (the crazy `#object[...]` name is how Clojure prints out
