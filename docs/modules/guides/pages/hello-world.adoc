...
tells Clojure what namespace to put the code into. If you're familiar with Java,
...
Because `hello` has required the `io.pedestal.connector` namespace, it can make use of functions in that namespace, such as `io.pedestal.connector/with-default-interceptors`.  This is the truest form, where every symbol in a namespace has a full name that includes the namespace.
