...
it uses; requiring a namespace loads that namespace's source and, optionally, creates an _alias_ for the namespace.
...
Because `hello` has required the `io.pedestal.connector` namespace, it can make use of functions in that namespace, such as `io.pedestal.connector/with-default-interceptors`.  This is the truest form, where every symbol
from a namespace has a full name that includes the namespace.
...
will translate the map into a full HTTP response complete with appropriate
...
just taking the easy road and letting Pedestal's defaults do the work.
