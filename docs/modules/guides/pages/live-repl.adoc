...
`routes-from` is built on api:dev-mode?[ns=io.pedestal.environment], a boolean value set from the JVM system property
`io.pedestal.dev-mode`.  By default, `dev-mode?` is false, and `routes-from` operates in production mode, so
`(route/routes-from routes)` evaluates to `routes`.

However, in development mode, `(route/routes-from routes)` expands to `#(expand-routes (deref (var routes)))` ... the
