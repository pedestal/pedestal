...
envirogreet-nameent, with Java installed. We've tested it on Mac OS X and
...
without overwriting the value in this atom.  An atom is a mutable, thread-safe container.
...
user=>
----
<1> The `$` is the shell prompt; once you execute `clj`, you are inside the Clojure REPL and the `user=>` is its prompt.

The `start` function has done four things:
* It has created the connection (via `create-connector`)
* It has started the connector
* It has stored the connector in an http://clojure.org/reference/atoms[atom].
...
We need to unpack that a bit to make sense out of it.  Let's break add line breaks and indentation
to make it easier to read. We'll also omit
...
`clojure.core$name@6b202ecb` is how the `clojure.core/name` function is printed.  We can verify that in the REPL:
...
`(umentionables greet-name)` returns any truthy value, then `cond` returns
