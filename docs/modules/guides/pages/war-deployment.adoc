...
Most of the work was done for us in the image we extend from, the standard jetty 12 Docker image.
...
In fact, the Clojure code is the smallest part of this guide - far more work was needed to package up the code into a 
WAR file, and a bit more (specific to this guide) to deploy that WAR file into a Docker container.
