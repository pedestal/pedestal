...
In this deployed mode, the servlet container will instantiate an instance of a Pedestal-provided
servlet class; when it initializes, the servlet will create a bridge from the Servlet world into the Pedestal world.  We'll cover the details below.
...
Even though this application doesn't used WebSockets, the Pedestal libraries _do_ have WebSocket support built-in, so these dependencies must be included, even if they won't be actively used.
...
- The servlet invokes `org.example.war.service/create-bridge` function when it initializes
...
libraries, such as `pedestal.servlet` and all the transitive dependencies. We can do this
...
In fact, the Clojure code in the smallest part of the effort - much work was needed to package up the code into a WAR file, and a bit more (specific to this guide) to deploy that WAR file into a Docker container.
