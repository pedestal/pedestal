...
`definterceptor` also quietly adds an implementation of api:IntoInterceptor[]; `defintercetptor` provides :enter, :leave, and :error keys
as necessary, to match the methods; the functions automatically invoke the corresponding method.  It _just works_.
