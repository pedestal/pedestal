...
The older prefix-tree router allows for route conflicts between static paths and those with path parameters, and silently
...
match the second route - because routes that do not contain the wild-card path parameter must match the exact number of terms.
