...
Pedestal that can operate before a connector map is used to create and start a connector.
...
For function config types, the value is converted to a fully qualified symbol and that is resolved
to a function.
...
=== io.pedestal.json-processor / PEDESTAL_JSON_PROCESSOR

- type: function
- default: api:processor[ns=io.pedestal.json.charred]

Defines what implementation of the io.pedestal.json.protocols/JSONProcessor to use; this function returns an instance 
of the JSONProcessor protocol.

This allows for either more involved use of the link:https://github.com/cnuernber/charred[charred] library, or 
replacing charred with something else, such as https://github.com/dakrone/cheshire[cheshire].
...
The default function gets tracing sources via  `io.opentelemetry.api.GlobalOpenTelemetry.getTracer(java.lang.String)`.
