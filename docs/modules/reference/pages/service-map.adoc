...
| Something that satisfies the api:expand-routes[ns=io.pedestal.http.route] as the full route map contains some redundancies to make processing easier which `expand-routes` adds automatically.
...
The service map may be validated via {clojure_spec}; the ::io.pedestal.http/service-map spec identifies
the possible keys and values.
