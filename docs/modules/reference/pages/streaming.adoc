...
This is operating at a very low level, as the interceptor pipeline has already completed.

In a traditional request, interceptors are resonsible for encoding, such as converting EDN data to JSON.
When supplying a channel, it is the process putting values into the channel that is responsible.
