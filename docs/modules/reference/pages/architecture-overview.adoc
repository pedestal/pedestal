...
WARNING: At this point, the Servlet Chain Provider approach is intended only for legacy Pedestal applications, or the rare cases where a Pedestal application must be deployed as a xref:guides:war-deployment.adoc[WAR].

The chain provider is a bridge between the Servlet API world
(HttpServlet, HttpServletRequest, and so forth) and Pedestal's model
(the interceptor chain, the request and response maps).

Pedestal includes a servlet chain provider out of the box, in
the io.pedestal/pedestal.servlet module. This provider connects
...
WARNING: This example is very out of date.
