...
instead return a {core_async} channel. Pedestal will yield the request processing thread and
...
| The list is evaluated and its result is used as an interceptor. footnote:[This is supported behavior
  related to the xref:table-syntax.adoc[table router syntax], but is no longer commonly used.]
...
Pedestal treats the handler as a function that takes a xref:request-map.adoc[] parameter,
