...
// TODO: This link is slightly broken
see api:*default-handler-names*[ns=io.pedestal.interceptor] for further details.
====
...
to the Var.
<2> Prior to release 0.8.0, this would be an error because there would be no route name; now the route name
will be the same as the interceptor's name.
