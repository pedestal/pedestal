...
From the `service` directory, execute `clj -T:build compile-java :aliases '[:local :servlet-api]'.
...
From the `tests` directory, execute `clj -T:tests run`.
...
4. Run `clj -T:tests run` from the `tests` directory.
